#ConnectionPool - 高新能C++MySQL数据库连接池
...
* **智能指针**：使用 `std::unique_ptr` 管理连接生命周期，并结合**自定义删除器**实现连接的“自动归还”而非真正销毁。

## 🔄 核心业务流程
1. **连接初始化**：启动时，连接池根据 `mysql.ini` 配置文件，预先创建 `initSize` 个数据库连接并放入队列中。
2. **生产者线程（连接自动扩容）**：当队列中的连接数少于底层要求，且总连接数未达到 `maxSize` 时，独立的守护线程会自动创建新连接。
3. **消费者（业务线程获取连接）**：业务代码调用接口获取连接，若队列为空，则阻塞等待 `connectionTimeout` 时间。
4. **回收与释放**：
...
| **拷贝构造 / 拷贝赋值** | `Connection(const Connection&) = delete;`<br>`operator=(const Connection&) = delete;` | `ConnectionPool(const Connection&) = delete;`<br>`operator=(const Connection&) = delete;` |
...
* **设计保障**：通过控制构造函数权限和静态局部变量，将连接池强制约束为**全局唯一单例**，实现全局资源的集中式资源共享与动态配额管理。
---
<details>
<summary>🔍 <b>点击展开：查看关于单例模式设计抉择与现代 C++ 线程安全源码剖析</b></summary>
...
std::shared_ptr<Connection> ConnectionPool::getConnection() {
    ...
    // ⚠️ 注意：这里不能使用 std::make_shared，因为常规构造无法传入自定义删除器
...
### 🚀 未来展望 (Next Steps)
虽然目前连接池的核心功能已经完全实现且具备高健壮性，但技术演进永无止境。在未来的迭代中，我计划从以下几个维度继续突破：
- [ ] **引入性能测试基准（Benchmark）**：使用 Pthread 或标准库线程模拟极致并发，利用 `std::chrono` 压测裸连接与连接池在不同并发量下的 QPS 提升对比，用真实数据说话。
- [ ] **连接队列的高效无锁化（Lock-Free）改造**：尝试使用无锁队列（如基于原子变量 `std::atomic` 实现的环形缓冲区），进一步压榨高并发下连接出入队列的性能死角。
- [ ] **动态扩缩容算法优化**：结合业务波峰波谷，设计更平滑的连接销毁衰减曲线，避免在临界值附近发生连接频繁创建与销毁的“线程抖动”现象。
